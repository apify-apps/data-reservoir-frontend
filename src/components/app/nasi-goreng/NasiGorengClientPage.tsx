...
interface NasiGorengClientPageState {
  pickedTable: NasiGorengTableType | null
}

export default function NasiGorengClientPage() {
  const [state, setState] = useState<NasiGorengClientPageState>({
    pickedTable: 'nasi_goreng_burned_food'
  });
  ...
  if (isLoading || !summaryData) return (<Loading/>)
    ...
    let onClickCategory = (pickedTable: string, enabled: boolean) => {
      setState(produce(s => {
        if (enabled) s.pickedTable = pickedTable as NasiGorengTableType
      ...
      <div className='flex flex-col gap-4 text-white'>
        ...
        <div className='grid grid-cols-4 gap-4 max-lg:grid-cols-2 max-lg:grid-rows-2'>
          ...
          <Paper className='px-4 col-span-2 p-4 text-xs max-lg:col-span-2'>
            <Picker options={Object.entries(NasiGorengTableLabel).map(([k, v]) => ({ label: v, value: k }))} onClickCategory={onClickCategory} singleOption selected={state.pickedTable}/>
          </Paper>
        </div>

        <div className='min-h-[100vh]'>
          {/* { state.pickedTable === "the_sims_castaway_product" && <CastawayProduct/> } */}
          { state.pickedTable === 'nasi_goreng_burned_food' && <BurnedFood/> }
