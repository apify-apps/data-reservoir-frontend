...
export default function FarmFrenzyTwoPizzaProduct() {
  const { isLoading, data } = useQuery({
    queryKey: ["farm-frenzy-two-pizza-product"],
    queryFn: async () => {
      let j = await request<FarmFrenzyTwoPizzaProductResponse[], {}>({
        method: "GET",
        url: API_ROUTE.FARM_FRENZY.TWO_PIZZA_PRODUCT,
      });
      return (j?.data ?? []);
    }
  });

  const colHelper = createColumnHelper<FarmFrenzyTwoPizzaProductResponse>();
  const columns = [
    colHelper.display({
      id: 'index',
      header: "#",
